...
// Run starts the H2APK HTTP server. It reads the working directory as baseDir,
// checks dependencies, registers routes, and blocks serving requests.
func Run() {
	baseDir, _ := os.Getwd()
...
func envVar(k, def string) string {
	if v := os.Getenv(k); v != "" {
		return v
	}
	return def
